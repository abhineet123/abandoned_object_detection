#ifndef _IMAGE_NEW_H_
#define _IMAGE_NEW_H_
...
public:
	...
	ImageBase(const ImageBase& rhs) {
		// it is very inefficient if this copy-constructor is called
		assert(false);
	}

	// assignment operator
	ImageBase& operator=(const ImageBase& rhs) {
		// it is very inefficient if operator= is called
		assert(false);
...
public:
	RgbImage(Mat img = NULL) : ImageBase(img) {
		;
	}
