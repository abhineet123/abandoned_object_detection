...
void ZivkovicAGMM::Initalize(const BgsParams& param) {
	...
	m_modes_backup = new GMM[m_params.Size()*m_params.MaxModes()];
...
void ZivkovicAGMM::UpdateParams(const BgsParams& param) {
	m_params = (ZivkovicParams&)param;	
}

void ZivkovicAGMM::PrintParams(){
	std::cout<<"\n";
...
void ZivkovicAGMM::copyModel(ZivkovicAGMM::GMM *src,ZivkovicAGMM::GMM *dst,int no_of_elements){	
	for(int i=0;i<no_of_elements;i++){
		...
		dst[i].sigma=src[i].sigma;
	}
}

void ZivkovicAGMM::Update(const RgbImage& data,  const BwImage& update_mask) {
   
}

void ZivkovicAGMM::Subtract2(const RgbImage& data,BwImage& low_threshold_mask, 
			   BwImage& high_threshold_mask, BwImage& static_threshold_mask,BwImage& update_mask,double static_threshold){

}

void ZivkovicAGMM::SubtractPixel2(long posPixel, const RgbPixel& pixel, unsigned char* pModesUsed,
                                  unsigned char& low_threshold, unsigned char& high_threshold,bool update_model) {
	...
	for (int iModes = 0; iModes < nModes; iModes++) {
		...
		if (!bFitsPDF) {
			...
			if (dist < m_params.LowThreshold()*var) {
				...
				// Sort weights so they are in descending order. Note that only the weight for this
...
void ZivkovicAGMM::Subtract2(const RgbImage& data,
	...
	/*if(update_mask(r,c)==FOREGROUND)
		copyModel(m_modes,m_modes_backup);*/
    ...
    for(unsigned int r = 0; r < m_params.Height(); ++r) {
        for(unsigned int c = 0; c < m_params.Width(); ++c) {
			...
			SubtractPixel2(posPixel, data(r,c), pUsedModes, low_threshold, high_threshold,update_model);
	...
	/*if(update_mask(r,c)==FOREGROUND)
		copyModel(m_modes_backup,m_modes);*/
}
...
///////////////////////////////////////////////////////////////////////////////
void ZivkovicAGMM::Subtract(const RgbImage& data,
...
void ZivkovicAGMM::pushIntoBackground(const CvRect bounding_box,RgbImage &current_frame){
...
void ZivkovicAGMM::resetBackground(RgbImage &current_frame,BwImage *reset_mask){
