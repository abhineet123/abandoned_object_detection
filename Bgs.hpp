...
public:
	...
	virtual void UpdateParams(const BgsParams& param)=0;
	...
	virtual void Subtract(const RgbImage& data,
		BwImage& low_threshold_mask, BwImage& high_threshold_mask) = 0;	

	virtual void Subtract2(const RgbImage& data,  
		BwImage& low_threshold_mask, BwImage& high_threshold_mask,BwImage& update_mask) = 0;	

	virtual void Subtract2(const RgbImage& data,BwImage& low_threshold_mask, 
		BwImage& high_threshold_mask, BwImage& static_threshold_mask,BwImage& update_mask,double static_threshold)=0;

	// Update the background model. Only pixels set to background in update_mask are updated.
	virtual void Update(const RgbImage& data,  const BwImage& update_mask) = 0;

	virtual void pushIntoBackground(const CvRect bounding_box,RgbImage &current_frame) = 0;

	virtual void resetBackground(RgbImage &current_frame,BwImage *reset_mask) = 0;
